#!/bin/sh
# run_all.sh - 모든 DB 수집/보완 스크립트를 순서대로 실행 (alpine sh 호환)
# 사용: docker exec -d my-backend sh scripts/run_all.sh
...
log ""
log "########## 4/6 상점 링크 수정 ##########"
node scripts/fix_store_urls.js --fix >> "$LOG" 2>&1
if [ $? -eq 0 ]; then log "✅ 상점 링크 완료"; else log "⚠️ 상점 링크 실패"; fi
