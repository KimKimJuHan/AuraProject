...
router.get('/games', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const appIds = topGames.map(g => g.appid); // 숫자 배열

        // ★ DB에서 조회 (steam_appid가 숫자든 문자든 매칭 시도)
        // Mongoose는 스키마 정의에 따라 자동 형변환을 시도하지만, 안전을 위해 명시적으로 조회
        const localGames = await Game.find({ steam_appid: { $in: appIds } })
            .select("steam_appid smart_tags title")
            .lean();

        // 4. 스팀 데이터 + 스마트 태그 병합
        const enrichedGames = sortedGames.map(g => {
            // ★ 중요: 데이터 타입(String vs Number) 차이로 인한 매칭 실패 방지 (== 사용)
            const match = localGames.find(lg => lg.steam_appid == g.appid);
            
            return { 
                ...g, 
                // DB에 정보가 있으면 그 태그를 쓰고, 없으면 빈 배열
