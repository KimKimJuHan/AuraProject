...
const styles = {
  ...
  tableContainer: { maxWidth: '1000px', margin: '0 auto', overflowX: 'auto', borderRadius: '10px', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' },
  table: { width: '100%', borderCollapse: 'collapse', backgroundColor: '#021E73', borderRadius: '10px', overflow: 'hidden' },
  th: { padding: '18px', backgroundColor: '#3D46F2', color: '#FFFFFF', textAlign: 'left', fontWeight: 'bold', fontSize: '1.1rem' },
  td: { padding: '20px', borderBottom: '1px solid #011526', verticalAlign: 'middle' },
  ...
  releaseDate: { fontSize: '0.9rem', color: '#AAAAAA' },
  priceContainer: { display: 'flex', flexDirection: 'column', justifyContent: 'center' },
  price: { color: '#A24CD9', fontWeight: 'bold', fontSize: '1.2rem' },
  discountBadge: { display: 'inline-block', backgroundColor: '#D94F4C', color: 'white', padding: '2px 6px', borderRadius: '4px', fontSize: '0.8rem', marginLeft: '8px', fontWeight: 'bold' },
  storeName: { fontSize: '0.9rem', color: '#D494D9', marginTop: '4px' },
  score: { fontWeight: 'bold', color: '#F2B705' },
  playtime: { fontSize: '0.9rem', color: '#5FCDD9', marginTop: '5px' },
  removeBtn: { padding: '8px 16px', backgroundColor: '#D94F4C', color: 'white', border: 'none', borderRadius: '999px', cursor: 'pointer', fontWeight: 'bold', transition: 'background 0.2s' },
        ...
        <table style={styles.table}>
          <thead>
            <tr>
              ...
              <th style={styles.th}>정보</th>
              <th style={styles.th}>가격 / 스토어</th>
              <th style={styles.th}>평가</th>
          ...
          <tbody>
            {wishlistGames.map(game => (
              <tr key={game.slug}>
                ...
                <td style={styles.td}>
                  <Link to={`/game/${game.slug}`} style={styles.gameTitle}>{game.title}</Link>
                  <div style={styles.releaseDate}>{game.releaseDate ? new Date(game.releaseDate).toLocaleDateString() : '-'} 출시</div>
                  <div style={{marginTop: '8px'}}>
                    {game.smart_tags?.slice(0, 2).map(tag => (
                      <span key={tag} style={{fontSize: '12px', color: '#ccc', marginRight: '5px', backgroundColor:'rgba(255,255,255,0.1)', padding:'2px 6px', borderRadius:'4px'}}>#{tag}</span>
                    ))}
                  </div>
                </td>
                <td style={styles.td}>
                  <div style={styles.priceContainer}>
                    {game.price_info?.isFree ? (
                      ...
                      <>
                        <div>
                          <span style={styles.price}>{game.price_info?.current_price?.toLocaleString()}원</span>
                          {game.price_info?.discount_percent > 0 && <span style={styles.discountBadge}>-{game.price_info.discount_percent}%</span>}
                        </div>
                ...
                <td style={styles.td}>
                  {game.metacritic_score > 0 ? <div style={styles.score}>M: {game.metacritic_score}점</div> : <div style={{color:'#666', fontSize:'0.9rem'}}>평점 없음</div>}
