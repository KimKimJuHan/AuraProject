...
const styles = {
  ...
  mediaStrip: { display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '10px' },
  thumbItem: { width: '120px', height: '68px', borderRadius: '2px', cursor: 'pointer', objectFit: 'cover', border: '2px solid transparent', opacity: 0.6 },
  thumbItemActive: { border: '2px solid #E50914', opacity: 1 },
  playButtonOverlay: { position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '60px', color: 'rgba(255,255,255,0.8)', cursor: 'pointer', zIndex: 10 },
    ...
    <div style={{ display:'flex', gap:'10px', overflowX:'auto', paddingBottom:'10px' }}>
      {games.map(game => (
        <a key={game.slug} href={`/game/${game.slug}`} style={{ minWidth:'150px', textDecoration:'none', color:'#fff', flexShrink:0 }}>
          <img src={game.main_image} alt={game.title} style={{ width:'150px', height:'84px', borderRadius:'4px', objectFit:'cover', marginBottom:'6px' }} />
  ...
  useEffect(() => {
    const fetchDetails = async () => {
        try {
            ...
            const data = res.data;
            setGameData(data); setLoading(false);
            ...
            if (combinedList.length > 0) { setSelectedMedia(combinedList[0]); setIsPlaying(false); }
        ...
        <div style={styles.galleryContainer}>
            <div style={styles.mainMediaDisplay}>
                {selectedMedia?.type === 'video' ? (
                    <>
                        <video ref={videoRef} src={selectedMedia.url} controls={isPlaying} style={{width:'100%', height:'100%', objectFit:'contain', display: isPlaying ? 'block' : 'none'}} />
                        {!isPlaying && <><img src={selectedMedia.thumb} alt="Poster" style={{width:'100%', height:'100%', objectFit:'cover', opacity:0.7}} /><div style={styles.playButtonOverlay} onClick={handlePlayVideo}>▶</div></>}
                    </>
                ) : (<img src={selectedMedia?.url} alt="Main" style={{width:'100%', height:'100%', objectFit:'contain'}} />)}
            </div>
            <div style={styles.mediaStrip}>
                {mediaList.map((item, idx) => (
                    <div key={idx} style={{position:'relative', flexShrink:0}} onClick={() => {setSelectedMedia(item); setIsPlaying(false);}}>
                        <img src={item.thumb} alt={`thumb`} style={{...styles.thumbItem, ...(selectedMedia?.url === item.url ? styles.thumbItemActive : {})}} />
                    </div>
                ))}
            </div>
        </div>
