...
const styles = {
  ...
  // ★ 클릭 가능한 스토어 행 (오류 해결)
  storeRowLink: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px', borderBottom: '1px solid #333', backgroundColor: '#181818', textDecoration: 'none', color: 'inherit', transition: 'background 0.2s', cursor: 'pointer', width: '100%', boxSizing: 'border-box' },
...
function ShopPage({ region }) { 
  ...
  gameData.trailers?.forEach(url => {
      ...
      allMedia.push({type:'video', url: embedUrl});
  });
      ...
      <div>
        <div style={styles.mainMediaDisplay}>
          {selectedMedia?.type === 'video' ? (
            <iframe src={selectedMedia.url} style={{width:'100%', height:'100%', border:'none'}} allow="autoplay; encrypted-media" allowFullScreen title="Trailer" />
          ) : (
